...
fn main() {
    ...
    let mut buf = Vec::new();
    let mut ext = std::path::Path::new(&path).extension().unwrap();
    f.read_to_end(&mut buf).unwrap();
    match transcode(buf, ext.to_str().unwrap(), "ogg", codec::id::Id::VORBIS) {
        Ok(f) => {
            play(f);
        }
        Err(e) => {
...
fn transcode(data: Vec<u8>,
             ict: &str,
             oct: &str,
             format: codec::id::Id)
             -> Result<Arc<Mutex<Vec<u8>>>, ffmpeg::Error> {
    use std::thread;
...
fn play(buffer: Arc<Mutex<Vec<u8>>>) -> shout::ShoutConn {
    ...
    loop {
        ...
        if pos + step < data.len() {
            conn.send(data[pos..(pos + step)].to_vec());
            pos += step;
            drop(data);
            conn.sync();
        } else {
            conn.send(data[pos..(pos + (data.len() - pos))].to_vec());
            println!("Finished!");
            break;
        }
    }
